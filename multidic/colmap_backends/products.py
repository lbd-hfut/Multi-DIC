...
def _robust_upper_mask(values: np.ndarray, mad_z: float) -> tuple[np.ndarray, dict[str, float]]:
    values = np.asarray(values, dtype=np.float64).reshape(-1)
    if len(values) == 0:
        return np.zeros(0, dtype=bool), {"median": 0.0, "mad_sigma": 0.0, "threshold": 0.0}
    median = float(np.median(values))
    mad = float(np.median(np.abs(values - median)))
    sigma = 1.4826 * mad
    if not np.isfinite(sigma) or sigma <= 1e-12:
        q1, q3 = np.percentile(values, [25.0, 75.0])
        sigma = float((q3 - q1) / 1.349) if q3 > q1 else 0.0
    if not np.isfinite(sigma) or sigma <= 1e-12:
        threshold = float(np.max(values))
    else:
        threshold = median + float(mad_z) * sigma
    return values <= threshold, {"median": median, "mad_sigma": float(sigma), "threshold": float(threshold)}


def _filter_sparse_points_by_3d_distribution(
    points_colmap: np.ndarray,
    ...
    opts = options or {}
    enabled = _as_bool(opts.get("enabled"), True)
    min_points = int(opts.get("min_points", 200))
    min_keep_ratio = float(opts.get("min_keep_ratio", 0.5))
    radius_mad_z = float(opts.get("radius_mad_z", 8.0))
    knn = int(opts.get("knn", 8))
    knn_mad_z = float(opts.get("knn_mad_z", 8.0))

    stats: dict[str, Any] = {
        "enabled": enabled,
        "input_points": int(len(points_colmap)),
        "retained_points": int(len(points_colmap)),
        "removed_points": 0,
        "min_points": min_points,
        "min_keep_ratio": min_keep_ratio,
        "radius_mad_z": radius_mad_z,
        "knn": knn,
        "knn_mad_z": knn_mad_z,
        "applied": False,
        "fallback_used": False,
    }
    if not enabled or len(points_colmap) < min_points:
        stats["reason"] = "disabled" if not enabled else "too_few_points"
        return points_colmap, point_ids, point_errors, stats

    points = np.asarray(points_colmap, dtype=np.float64).reshape(-1, 3)
    center = np.median(points, axis=0)
    radii = np.linalg.norm(points - center.reshape(1, 3), axis=1)
    radius_mask, radius_stats = _robust_upper_mask(radii, radius_mad_z)

    try:
        from scipy.spatial import cKDTree

        query_k = min(max(2, knn + 1), len(points))
        dists, _ = cKDTree(points).query(points, k=query_k)
        kth_dist = np.asarray(dists[:, -1], dtype=np.float64)
        density_mask, density_stats = _robust_upper_mask(kth_dist, knn_mad_z)
    except Exception as exc:
        density_mask = np.ones(len(points), dtype=bool)
        density_stats = {"error": f"{type(exc).__name__}: {exc}"}

    keep = radius_mask & density_mask
    min_keep = int(np.ceil(min_keep_ratio * len(points)))
    if int(np.count_nonzero(keep)) < max(1, min_keep):
        stats.update(
            {
                "reason": "fallback_min_keep_ratio",
                "fallback_used": True,
                "radius_filter": radius_stats,
                "density_filter": density_stats,
                "candidate_retained_points": int(np.count_nonzero(keep)),
            }
        )
        return points_colmap, point_ids, point_errors, stats
    ...
    return points_colmap[keep], point_ids[keep], point_errors[keep], stats
...
def _export_ndef_sfm_products(
    ...
    max_reproj_error: float,
    dpi: int,
    spatial_outlier_filter: dict[str, Any] | None = None,
) -> dict[str, str]:
    ...
    points_colmap, point_ids_arr, point_errors_arr, spatial_filter_stats = _filter_sparse_points_by_3d_distribution(
        points_colmap,
        point_ids_arr,
        point_errors_arr,
    ...
    np.savez(
        ...
        spatial_outlier_removed=np.array(spatial_filter_stats["removed_points"], dtype=np.int64),
    )
    np.savez(
    ...
    savemat(
        paths.workspace / "points3D.mat",
        {
            ...
            "spatial_outlier_removed": spatial_filter_stats["removed_points"],
        },
    )
    with (paths.workspace / "sparse_point_filter.json").open("w", encoding="utf-8") as handle:
        json.dump(spatial_filter_stats, handle, ensure_ascii=False, indent=2)
        handle.write("\n")
    ...
    return {
        ...
        "sparse_point_filter_json": str(paths.workspace / "sparse_point_filter.json"),
